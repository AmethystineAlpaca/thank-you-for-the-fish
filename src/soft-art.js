...
(function () {
  ...
  // Alternate forms use a warmer, mixed style bank instead of clustering in cool hues.
  // Warm / tropical / natural / jewel families are deliberately mixed; all choices stay stable per species.
  const alternateLooks = [
    { hue: 8, s: .46, lift: .018 },    // terracotta
    { hue: 18, s: .52, lift: .024 },   // coral
    { hue: 29, s: .48, lift: .030 },   // apricot
    { hue: 40, s: .50, lift: .028 },   // honey amber
    { hue: 52, s: .44, lift: .024 },   // antique gold
    { hue: 67, s: .39, lift: .020 },   // ochre
    { hue: 92, s: .34, lift: .024 },   // warm sage
    { hue: 150, s: .38, lift: .028 },  // soft jade
    { hue: 176, s: .42, lift: .020 },  // muted turquoise
    { hue: 207, s: .44, lift: .018 },  // denim blue
    { hue: 238, s: .42, lift: .024 },  // cornflower
    { hue: 282, s: .40, lift: .020 },  // plum
    { hue: 320, s: .46, lift: .024 },  // berry
    { hue: 346, s: .48, lift: .018 },  // garnet rose
    { hue: 24, s: .43, lift: .018 }    // copper
  ];
  ...
  function prismStops(look, id) {
    ...
    // Curated harmony families: analogous, triadic, split-complementary, tetradic and wide-spectrum.
    // The family and root shift are stable per species, so the palette feels varied without flickering.
    const harmonyBank = [
      [-32, -14, 6, 28, 52],
      [0, 60, 120, 180, 240],
      [0, 120, 240, 30, 210],
    ...
    const rootShifts = [-90, -60, -30, 0, 30, 60, 90, 120, 180];
    const offsets = harmonyBank[Math.floor(rand() * harmonyBank.length)];
    const root = (look.palette.hue + rootShifts[Math.floor(rand() * rootShifts.length)] + (rand() - .5) * 18 + 360) % 360;

    function build(phase) {
      return offsets.map((off, i) => {
        const h = (root + off + phase + (rand() - .5) * 10 + 360) % 360;
        const s = .68 + rand() * .18;
        const l = .57 + rand() * .12;
        const a = i === offsets.length - 1 ? 0 : (.80 - rand() * .11);
  ...
  function appearance(source, f) {
    ...
    const alternateLook = alternateLooks[hashInt('alternate-' + f.id) % alternateLooks.length];
    // Nudge the curated hue slightly according to the source species so results do not feel preset-flat.
    const sourceNudge = Math.max(-12, Math.min(12, hueDistance(dominant, palette.hue) * .18));
    const alternateHue = (alternateLook.hue + sourceNudge + 360) % 360;

    const result = { palette, alternateHue, alternateLook, bounds, alternate: null, glows: new Map() };
  ...
  function alternateSource(source, look) {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      const target = (look.alternateHue + Math.max(-24, Math.min(24, hueDistance(h, look.palette.hue) * .20)) + 360) % 360;
      // Softer saturation prevents the synthetic highlighter-green / crushed-magenta look.
      const sat = Math.min(look.alternateLook.s, Math.max(.28, s * .72));
      const light = Math.min(.84, Math.max(.14, l + look.alternateLook.lift));
      const color = rgb(target, sat, light);
  ...
  function luminousSource(source, look) {
    ...
    for (let y = 0; y < h; y++)for (let x = 0; x < w; x++) {
      ...
      lightData[i + 3] = alpha * (.042 + .264 * detail + .156 * highlight);
      for (let j = 0; j < 3; j++) {
  ...
  function foilReflection(canvas, c, source, look, x, y, dw, dh, t, kind, id) {
    ...
    c.save(); c.globalCompositeOperation = 'color'; c.globalAlpha = .20; c.drawImage(foil, 0, 0);
    c.globalCompositeOperation = 'soft-light'; c.globalAlpha = .336; c.drawImage(foil, 0, 0); c.restore();
    // Strong colored prism emission — use the actual rainbow layer before it is cleared.
    c.save();
    c.globalCompositeOperation = 'lighter';

    // Wide saturated colored glow
    c.globalAlpha = .302;
    c.filter = 'saturate(2.0) brightness(1.65) blur(7px)';
    c.drawImage(foil, 0, 0);

    // Tighter colored glow
    c.globalAlpha = .346;
    c.filter = 'saturate(1.8) brightness(1.55) blur(2.5px)';
    c.drawImage(foil, 0, 0);

    // Hot luminous color directly on the body
    c.globalCompositeOperation = 'screen';
    c.globalAlpha = .274;
    c.filter = 'saturate(1.65) brightness(1.5)';
    c.drawImage(foil, 0, 0);
  ...
  function fish(canvas, f, trait = 'normal', t = 0, catchRecord = null) {
    ...
    if (trait === 'glow') {
      ...
      c.save(); c.globalCompositeOperation = 'screen'; c.globalAlpha = Math.min(1, pulse * 1.18); c.filter = 'brightness(1.58) saturate(1.14)';
      Motion.draw(c, look.emission, x, y, dw, dh, t, kind); c.restore();
      c.save(); c.globalAlpha = Math.min(1, pulse * 1.16);
      const radius = Math.max(.8, dw * .009);
      c.filter = `brightness(1.54) drop-shadow(0px 0px ${radius}px ${look.neonColor}) drop-shadow(0px 0px ${radius * 2}px ${look.neonColor})`;
      Motion.draw(c, look.rim, x, y, dw, dh, t, kind); c.restore();
      c.save(); c.globalAlpha = .82 * pulse; c.filter = 'brightness(1.49)';
